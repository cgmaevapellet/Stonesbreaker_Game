...
public class GameController : MonoBehaviour 
{
	...
	void Awake () 
	{
		bossAudio = transform.Find ("SecondaryMusic").GetComponent<AudioSource>();
	...
	void MusicFading()
	{
		...
		{
			GetComponent<AudioSource>().volume = Mathf.Lerp (GetComponent<AudioSource>().volume, 0.0f, musicFadeSpeed * Time.deltaTime);
			bossAudio.volume = Mathf.Lerp (bossAudio.volume, 0.05f, musicFadeSpeed * Time.deltaTime);
		}

		else
		{
			GetComponent<AudioSource>().volume = Mathf.Lerp (GetComponent<AudioSource>().volume, 0.1f, musicFadeSpeed * Time.deltaTime);
